;;;; -*- mode: lisp -*-
;;;; Benchmark: conskivi-inmemory vs Redis 7.0.15

(in-package #:cl-user)

(defpackage #:conskivi-benchmark
  (:use #:cl
        #:conskivi-core
        #:conskivi-inmemory)
  (:export #:run-benchmark))

(in-package #:conskivi-benchmark)
...
(defun print-header ()
  (format t "~%=== conskivi-inmemory vs Redis 7.0.15 Benchmark ===~%")
  (format t "Striped locks: ~d stripes~%~%" 256))
...
(defun run-benchmark (&key (iterations 100000) (threads-list '(1 2 4 8)))
  (let ((database (make-instance 'conskivi-inmemory-database
                                 :db-path #p"/tmp/conskivi-bench.ck")))
    (unwind-protect
         (progn
